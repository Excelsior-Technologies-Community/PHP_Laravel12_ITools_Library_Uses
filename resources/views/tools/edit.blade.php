...
<head>
    ...
    <title>Edit Tool - {{ $tool->name }}</title>
    ...
    <div class="max-w-3xl mx-auto py-12 px-4">
        <div class="bg-white shadow-2xl rounded-3xl p-8 border border-gray-100">
            ...
            <div class="flex justify-between items-center mb-10">
                <div>
                    <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight">✏️ Edit Tool</h2>
                    <p class="text-gray-500 mt-1">Update details for <b>{{ $tool->name }}</b></p>
                </div>
            ...
            <form action="{{ route('tools.update', $tool->id) }}" method="POST" class="space-y-6">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="category" value="{{ old('category', $tool->category) }}" required
                            class="w-full border border-gray-200 rounded-2xl px-5 py-4 focus:ring-4 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all placeholder-gray-300 shadow-sm" 
                ...
                <div>
                    ...
                    <textarea name="description" rows="5" required
                        ...
                        placeholder="What does this tool do?">{{ old('description', $tool->description) }}</textarea>
                </div>

                <div class="pt-4 flex items-center gap-4">
                    <button type="submit" class="flex-grow bg-green-600 hover:bg-green-700 text-white font-black py-4 rounded-2xl shadow-xl hover:shadow-green-200 transform hover:-translate-y-1 transition-all flex justify-center items-center gap-2">
                        <i class="fa-solid fa-circle-check"></i> Update Tool Details
                    </button>
